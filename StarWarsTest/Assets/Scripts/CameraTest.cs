...
public class CameraTest : MonoBehaviour {
	...
	void LateUpdate () {
		 if (Input.GetAxis("Vertical") != 0 )
		{
			...
			y += Input.GetAxis("Vertical") * mouseYSpeedMod;
			float targetRotantionAngle = CameraTarget.localEulerAngles.y;
			float cameraRotationAngle = transform.localEulerAngles.y;
		...
		Vector3 position = CameraTarget.localPosition - (rotation * Vector3.forward * desireDistance);

		RaycastHit collisionHit;
		Vector3 cameraTargetPosition = new Vector3 (CameraTarget.localPosition.x, CameraTarget.localPosition.y + cameraTargetHeight, CameraTarget.localPosition.z);
		...
		position = CameraTarget.localPosition - (rotation * Vector3.forward * currentDistance + new Vector3 (0, -cameraTargetHeight, 0));
		...
		float cameraX = transform.localRotation.x;

		CameraTarget.localEulerAngles = new Vector3(cameraX,transform.localEulerAngles.y,transform.localEulerAngles.z);
