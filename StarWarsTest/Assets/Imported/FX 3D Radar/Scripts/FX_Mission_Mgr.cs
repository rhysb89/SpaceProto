...
public class FX_Mission_Mgr : MonoBehaviour {
	...
	public void SetMissionActive(int m){
		for(int i = 0; i < MissionList.Count; i++){
			...
			MissionList[i].Mission.MissionNAVList[0].NAVPoint.GetComponent<FX_3DRadar_RID>().IsActiveNAV = true;
		}
	...
	public void SetMissionInactive(int m){
		...
		for(int i = 0; i < MissionList.Count; i++){
			...
			for(int a = 0; a < MissionList[i].Mission.MissionNAVList.Count; a++){
				MissionList[i].Mission.MissionNAVList[a].NAVPoint.GetComponent<FX_3DRadar_RID>().IsActiveNAV = false;
			}
		}
	}
}
